...
When people want to learn a new technique, or understand a new field, looking for related references is a very common way to do it. However, this process can be extremely irritating. There could be tremendous amounts of paper related to that techinque, describing how to apply or improve it under centain circumstances. It's hard to understand these papers without the basic knowledge of the histroy about this techinque. 

This project aims to provide a way to analyze the developing process of certain statistical techinques. When a key word such as **lasso** is provided, a summary for it will be produced, including how it was proposed in the first place and when it became widely applied in different fields. 


To attain this goal, we study [**Semantic Scholar Data**](http://s2-public-api-prod.us-west-2.elasticbeanstalk.com/corpus/). This is a large data set containing 220 million papers from various fields. To study the developing process of statistical techinques, the first step is filtering: we need to focus on mainstream academic journals that develope statistical methods or apply them. 

This blog post illustrates how to acheive the filterling process via [**vsp**](https://arxiv.org/pdf/2004.05387.pdf) algorithm.
...
The top ten journals that cite others most are more interesting. Actually, "arxiv" and "biorxiv" are not even journals, but they are sources for lots of papers, so it is not suprising that they cite others a lot. 
...
## Visualization
Here we provide a visualization for citation relationships between the selected journal groups. Flow between two blocks describes the left cluster citing the right, and the wider flow means more citations. This is an iteractive graph, move the mouse over flows and explore more!
